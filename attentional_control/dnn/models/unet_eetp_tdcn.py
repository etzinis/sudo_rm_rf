...
class MergeConvNorm(nn.Module):
    def __init__(self, B=256, branches=2, kernel_size=3):
        super().__init__()
        self.net = nn.Sequential(
            ConvNormAct(branches*B, B, 1, stride=1, groups=1),
            DilatedConvNorm(B, B, kSize=kernel_size, stride=1, groups=B, d=1),
            ConvNormAct(B, B, 1, stride=1, groups=1)
        )
...
class DownsamplerConvNorm(nn.Module):
    def __init__(self, B=256, H=512, stride=1):
        super().__init__()
        self.net = nn.Sequential(
            ConvNormAct(B, H, 1, stride=1, groups=1),
            DilatedConvNorm(H, H, kSize=2*stride+1, stride=stride,
                            groups=H, d=1),
            ConvNormAct(H, B, 1, stride=1, groups=1)
        )
...
class SeparableDilatedConvNorm(nn.Module):
    def __init__(self, B=256, H=512, kernel_size=3, stride=1, dilation=1):
        super().__init__()
        self.module_act = NormAct(B)
        self.net = nn.Sequential(
            ConvNormAct(B, H, 1, stride=1, groups=1),
            DilatedConvNorm(H, H, kSize=kernel_size, stride=stride,
                            groups=H, d=dilation),
            ConvNormAct(H, B, 1, stride=1, groups=1)
        )

    def forward(self, inp):
        return self.module_act(self.net(inp) + inp)


class MergeScales(nn.Module):
    def __init__(self, B=256):
        super().__init__()
        self.merge = ConvNormAct(2*B, B, 1, stride=1, groups=1)
        self.upsampler = torch.nn.Upsample(scale_factor=2)

    def forward(self, x_high_res, x_low_res):
        return self.merge(torch.cat([x_high_res, self.upsampler(x_low_res)], 1))


class AddScales(nn.Module):
    def __init__(self):
        super().__init__()
        self.upsampler = torch.nn.Upsample(scale_factor=2)

    def forward(self, x_high_res, x_low_res):
        return x_high_res + self.upsampler(x_low_res)
...
class EETPTDCN(nn.Module):
    ...
    def __init__(self, N, L, B, H, P, X, R, S=1):
        ...
        self.scales_modules = nn.ModuleList([
            nn.Sequential(
                *[SeparableDilatedConvNorm(B=B, H=H, kernel_size=P,
                                           dilation=dilation+1)
                  for dilation in range(X)])
        ])
        for r in range(1, R):
            this_scale_modules = nn.Sequential(
                DownsamplerConvNorm(B=B, H=H, stride=2),
                *[SeparableDilatedConvNorm(B=B, H=H, kernel_size=P,
                                           dilation=dilation+1)
                  for dilation in range(X // (r+1))])
            self.scales_modules.append(this_scale_modules)
        ...
        if B != N:
            self.reshape_before_masks = ConvNormAct(B, N, 1, stride=1, groups=1)
        ...
        self.ln_mask_in = NormAct(self.N)
    ...
    def forward(self, x):
        ...
        for r in range(self.R):
            x = self.scales_modules[r](x)
            # print('asdfsdafsadf', r)
            # print(self.scales_modules[r])
            outputs_in_scales.append(x)

        for r in range(self.R-1):
            low_resolution = outputs_in_scales.pop(-1)
            outputs_in_scales[-1] = self.scales_merging[r](
                outputs_in_scales[-1], low_resolution)

        x = outputs_in_scales[-1]
